...
class Genre {
    ...
    async create(name) {
        ...
        const [rows] = await pool.query("INSERT INTO genre(name) VALUES(?)", [name]);
        //console.log(rows);
    ...
    async update(id, name) {
        ...
        const [rows] = await pool.query("UPDATE genre SET name = ? WHERE id = ?", [name, id]);
        if (rows.affectedRows === 1) {
            return true;
        }
    ...
    async delete(id) {
        ...
        const [rows] = await pool.query("DELETE FROM genre WHERE id = ?", [id]);
        if (rows.affectedRows === 1) {
            return true;
        }
